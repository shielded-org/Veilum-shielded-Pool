...
/** True when Horizon has a ledger entry for this address (account is funded / exists). */
export async function isAccountOnChain(horizonUrl: string, address: string): Promise<boolean> {
  const base = horizonUrl.replace(/\/$/, "");
  const res = await fetch(`${base}/accounts/${address}`, { cache: "no-store" });
  if (res.status === 404) return false;
  if (!res.ok) throw new Error(`Failed to check account (${res.status})`);
  return true;
}
