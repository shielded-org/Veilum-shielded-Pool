...
/** Extension vault: deterministic seed from Stellar secret key (not wallet signature). */
export async function keySeedFromStellarSecret(secretKey: string): Promise<`0x${string}`> {
  const enc = new TextEncoder();
  const data = new Uint8Array([
    ...enc.encode("veilum-wallet-seed-v1"),
