...
export async function requestFaucetMint(payload: {
  ...
  const { relayerUrl } = await getServiceUrls();
  const url = `${relayerUrl.replace(/\/$/, "")}/faucet/mint`;
  let res: Response;
  try {
    res = await fetch(url, {
      method: "POST",
    ...
    const hint =
      error instanceof TypeError
        ? `Network error reaching relayer (${url}). Reload the extension after rebuilding, or set VITE_RELAYER_URL in apps/wallet-extension/.env.`
        : "Network error reaching relayer";
    throw new Error(hint);
  }
  const body = (await res.json()) as RelayerResponse;
  if (!res.ok) throw new Error(body.error || `Faucet HTTP ${res.status}`);
  if (body.status === "failed") throw new Error(body.error || "Faucet mint failed");
  return body;
}
