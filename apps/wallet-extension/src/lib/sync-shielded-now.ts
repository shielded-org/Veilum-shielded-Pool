...
import { isAccountOnChain } from "./friendbot";
...
import { formatWalletError } from "./wallet-error";
...
function resolveActiveWallet(): string | null {
  return useShieldedStore.getState().keyMaterialAddress;
}
...
export async function finishShieldedTransaction(
  ...
  if (!balanceVerified) {
    options.onStatus(
      "Transaction confirmed on-chain — balance is still syncing. Activity will update shortly."
    );
  }
...
async function runFullScan(options: SyncOptions, applyGen: number): Promise<boolean> {
  ...
  try {
    ...
    if (config.horizonUrl) {
      const onChain = await isAccountOnChain(config.horizonUrl, wallet);
      if (!onChain) {
        state.setNotes([]);
        state.setMerkleLeaves([]);
        state.setShieldedBalance(0n);
        state.setNotesChainReady(true);
        state.setSyncError(null);
        state.setSyncWarnings([]);
        return false;
      }
    }
    ...
    if (!awaitNullifiers && result.chainVerified && result.notes.length > 0 && !background) {
      ...
      void refreshNoteSpendStatus({
        ...
        .then((resolved) => {
          ...
          live.setShieldedBalance(shieldedTotal(verified));
