...
const ENV_BY_PKG: Record<string, string> = {
  hash3: "HASH3_DIR",
...
export function resolveNoirPackage(packageName: string): string {
  const envKey = ENV_BY_PKG[packageName] ?? `NOIR_${packageName.toUpperCase().replace(/-/g, "_")}_DIR`;

  const explicit = process.env[envKey];
  if (explicit && existsSync(join(explicit, "Nargo.toml"))) return explicit;

  const root = process.env.NOIR_PACKAGES_ROOT;
  if (root) {
    const fromRoot = join(root, packageName);
    if (existsSync(join(fromRoot, "Nargo.toml"))) return fromRoot;
  }
  ...
  for (const dir of candidates) {
    if (existsSync(join(dir, "Nargo.toml"))) return dir;
  }
  ...
  throw new Error(
    `Noir package "${packageName}" not found (set ${envKey} or NOIR_PACKAGES_ROOT). Is Nargo installed?`
  );
}
